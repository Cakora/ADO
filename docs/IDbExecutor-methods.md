...
| QueryTableAsync | DataTable | ❌ | ✅ | ✅ | ✅ | Provider DataAdapter.Fill(DataTable) | SQL/PG single SELECT buffered; Oracle/PG refcursor handled via adapter. |
| QueryAsync\<T\> | List\<T\> | ❌ | ✅ | ✅ | ✅ | QueryTableAsync + DataTable→List | Caller supplies Func\<DataRow, T\>; buffered. |
| ExecuteDataSetAsync | DataSet | ❌ | ✅ | ✅ | ✅ | Provider DataAdapter.Fill(DataSet) | SQL multi-SELECT; Oracle REF CURSOR; PG refcursor/multi-SELECT. |
| QueryMultipleAsync | MultiResult | ❌ | ✅ | ✅ | ✅ | ExecuteDataSetAsync + DataSet→MultiResult | Includes output parameters captured during fill. |
...
- Buffered multi: ExecuteDataSetAsync / QueryMultipleAsync — highest memory; required for multiple result sets and Oracle/PG cursor scenarios.
