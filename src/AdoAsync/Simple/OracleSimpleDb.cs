...
public sealed class OracleSimpleDb : IDisposable
{
    ...
    private static OracleCommand CreateCommand(OracleConnection connection, string commandText, CommandType commandType, int? commandTimeoutSeconds)
    {
    ...
    private static IReadOnlyList<SimpleParameter>? AddParameters(OracleCommand command, CommandType commandType, IEnumerable<SimpleParameter>? parameters)
    {
        ...
        foreach (var parameter in list)
        {
            ...
            {
                oracleParameter.OracleDbType = OracleTypeMapper.Map(parameter.DataType.Value);
            }

            if (parameter.Size.HasValue)
            {
                oracleParameter.Size = parameter.Size.Value;
            }
    ...
    private static IReadOnlyDictionary<string, object?> ExtractOutputs(OracleCommand command, IReadOnlyList<SimpleParameter>? parameters)
    {
    ...
    private static void ValidateRefCursorUsage(CommandType commandType, IReadOnlyList<SimpleParameter> parameters)
    {
        if (commandType != CommandType.StoredProcedure)
        {
            foreach (var parameter in parameters)
            {
                ...
                {
                    throw new DatabaseException(ErrorCategory.Validation, "Oracle refcursor outputs require CommandType.StoredProcedure.");
                }
            }
        }
    }
