# Implementation Workplan (Stepwise, No-Miss Checklist)

This file is the operational checklist for implementing the library described in `IMPLEMENTATION_GUIDE.md:1`.

Rule: **Do not start or continue any implementation step until the “Preflight” section is completed and the step’s “Exit criteria” are met.**

---

## Preflight (must do every time before coding)

- [x] Read `IMPLEMENTATION_GUIDE.md:1` fully and confirm there are no conflicting requirements.
- [x] Confirm decisions are locked:
  - [x] Async-first and async-only public API
  - [x] Executor is `IAsyncDisposable`; consumers use `await using` (no manual Dispose calls)
  - [x] Transactions: **Approach A** (explicit `DbTransaction` + rollback-on-dispose handle)
  - [x] Retry: central but controllable (`EnableRetry`), Polly-backed when enabled, off by default
  - [x] Validation: FluentValidation, on by default
  - [x] Diagnostics: off by default, returned via result contract (no logging)
  - [x] Multi-result support: **required**
  - [x] Custom error contract: provider exceptions must not escape as the public contract
  - [x] Provider support: SQL Server, PostgreSQL, Oracle
  - [x] `DbDataType` set is “complete, cross-provider” (includes JSON, XML, binary, LOBs, date/time variants, interval)
- [x] Reconfirm global MUST NOTs:
  - [x] No ORM / no Dapper / ADO.NET only
  - [x] No DI containers
  - [x] No static global helpers / no global mutable state
  - [x] No reflection-based magic
  - [x] No hidden retries / hidden transactions / hidden logging
  - [x] No provider-specific logic outside Providers
- [x] Tooling/quality gates prepared:
  - [x] Nullable enabled
  - [x] .NET analyzers enabled + warnings-as-errors
  - [x] SonarQube run configured (CI/local) and findings are treated as blocking
  - [x] `dotnet pack` / NuGet validation considered blocking

Exit criteria:
- You can point to the exact section(s) in `IMPLEMENTATION_GUIDE.md:1` that define each decision above.

---

## Traceability rule (how we “don’t miss any line/word”)

For every implementation PR/change set:
- [x] Add a short “Requirements touched” list mapping changes to headings in `IMPLEMENTATION_GUIDE.md:1`.
- [x] Confirm no change violates any “Global MUST NOT” rule.
- [x] If you add a new behavior, ensure it is documented (XML docs for public API; guide updates if it’s a new rule).
Note: latest mapping is tracked in `docs/requirements-touched.md`.

---

## Step-by-step implementation (execute in order)

### Step 1 — Solution + project (single class library supporting all providers)

Do:
- [x] Consolidate to one class library (single project) that supports SQL Server, PostgreSQL, and Oracle.
- [x] Remove/decommission extra projects from the solution so only the core library (and optional tests/demo) remain.
- [x] Keep provider-specific code encapsulated in provider folders/namespaces inside the project; no external provider projects.
- [x] Add/keep central build props/targets (e.g., `Directory.Build.props`) enabling nullable, analyzers, and warnings-as-errors across the solution.
- [x] Ensure any demo/test project references only this single library.

Exit criteria:
- `dotnet build -c Release` passes.
- Solution contains the single class library plus optional tests/demo; no leftover provider/Common projects.
- Provider logic lives in folders/namespaces inside the single library; no cross-project dependencies.

---

### Step 2 — Configuration contracts (`DbOptions`) with explicit defaults

Do:
- [x] Implement `DbOptions` with explicit defaults:
  - [x] `EnableDiagnostics = false`
  - [x] `EnableValidation = true`
  - [x] `EnableRetry = false`
  - [x] `RetryCount = 3`
  - [x] `RetryDelayMilliseconds = 200`
- [x] Ensure `DbOptions` contains all required fields and has no logic/environment reads.

Exit criteria:
- A consumer can create options by setting only `DatabaseType`, `ConnectionString`, and `CommandTimeoutSeconds`.

---

### Step 3 — Custom error contract (middleware + multi-language)

Do:
- [x] Define `DbError`, `DbErrorType`, `DbErrorCode` and localization fields (`MessageKey`, parameters, optional localized message).
- [x] Ensure all failures map to this contract (raw provider exceptions must not escape as public contract when using result envelopes).

Exit criteria:
- Error is serializable and stable for middleware.

---

### Step 4 — `DbDataType` enum + parameter model (+ Size/Precision/Scale rules)

Do:
- [x] Implement the full cross-provider `DbDataType` set from `IMPLEMENTATION_GUIDE.md:1`.
- [x] Ensure the enum covers strings (Unicode/ANSI, fixed/variable, CLOB/NClob), numbers (signed/unsigned with validation), boolean, GUID, binary (binary/blob), date/time family (Date, Time, DateTime, DateTime2, DateTimeOffset, Timestamp, Interval), and structured types (Json, Xml, Currency).
- [x] Implement `DbParameter` and enforce:
  - [x] Output params require `Size` (strings/binary)
  - [x] Decimals support `Precision/Scale`
  - [x] Encourage `Size` for variable-length input params when known (e.g., 400)

Exit criteria:
- Validation can reject missing `Size` for output and invalid unsigned usage where unsupported.

---

### Step 5 — Command model (`CommandDefinition`) + SQL injection rules

Do:
- [x] Support `CommandType.Text` and `CommandType.StoredProcedure` explicitly.
- [x] Ensure values must be passed via parameters; no helpers that encourage string interpolation.
- [x] Document and enforce identifier whitelisting responsibility.
- [x] Require stored procedure names/identifiers to come from controlled allow-lists (no raw user input).

Exit criteria:
- Stored procedure rules (command type, output params, provider ownership) are met.

---

### Step 6 — Result strategy + multi-result support (streaming default)

Do:
- [x] Implement streaming-first query shape (`IAsyncEnumerable<T>`).
- [x] Implement explicit materialization (`List<DataTable>` and optional `DataSet`).
- [x] Implement multi-result support (required) with:
  - [x] Streaming sequential results (`NextResultAsync`)
  - [x] Materialized multi-tables return

Exit criteria:
- Streaming does not buffer entire results; materialization is explicit and documented as allocation-heavy.

---

### Step 7 — Abstractions (interfaces only)

Do:
- [x] Define `IDbExecutor`, `IDbProvider`, `ITransactionManager` with no logic.
...
### Step 8 — Validation layer (FluentValidation)

Do:
- [x] Add validators for options, command, parameters.
- [x] Ensure validation runs only when `EnableValidation = true`.
- [x] Map validation failures to `DbErrorType.ValidationError` and never retry them.

Exit criteria:
- Invalid inputs fail fast before opening the connection/command execution.

---

### Step 9 — Resilience layer (Polly; optional via `EnableRetry`)

Do:
- [x] Build Polly retry policy explicitly from `DbOptions`.
- [x] Retry only transient categories; respect cancellation.
- [x] Define and enforce rule: do not retry inside an explicit user transaction.

Exit criteria:
- Retry behavior is predictable, opt-in, and recorded in diagnostics/result.

---

### Step 10 — Transactions (Approach A)

Do:
- [x] Implement explicit `Begin/Commit/Rollback`.
- [x] Add rollback-on-dispose transaction handle (async + sync wrapper).
- [x] Ensure same connection is reused for the entire transaction.

Exit criteria:
- Call sites do not need `try/catch/rollback`; rollback happens on dispose unless committed.

---

### Step 11 — Providers (SQL Server / PostgreSQL / Oracle)

Do (each provider):
- [x] Map `DbDataType` → provider parameter types correctly.
- [x] Translate provider exceptions to `DbError` categories/codes.
- [x] Handle stored procedure differences provider-side (Oracle cursors, etc.).
- [x] Ensure no provider code leaks into Core.

Exit criteria:
- Same consumer usage works for all providers; only `DbOptions.DatabaseType` changes.

---

### Step 12 — Executor (`DbExecutor`) facade (orchestrator only)

Do:
- [x] Lazy connection creation/open, close on dispose (no finalizers).
- [x] Expose minimal public API shapes: execute, scalar, streaming query, explicit materialization, multi-result.
- [x] Thread-safety: not thread-safe; sequential reuse allowed; concurrent use forbidden.

Exit criteria:
- End-to-end path: Consumer → Executor → Validation → Resilience → Provider → ADO.NET.

---

### Step 13 — IntelliSense/XML docs (mandatory)

Do:
- [x] Add XML docs on all public APIs with safe-by-default examples.
- [x] Ensure docs state memory/security/transaction/retry/cancellation behavior.

Exit criteria:
- IDE tooltips are usable and prevent common misuse.

---

### Step 14 — Tests (separate project)

Do:
- [x] Add tests: positive/negative, timeout+retry, transaction rollback, multi-result.
- [x] Add Oracle cursor exhaustion test strategy (as feasible).
...
### Step 15 — Quality gates + packaging (every change set)

Do:
- [x] Run analyzers (warnings-as-errors).
- [x] Run SonarQube analysis and fix findings.
- [x] Run `dotnet pack` and treat packaging validation failures as blocking.

Exit criteria:
- CI would fail if any violation exists.

---

### Step 16 — Optional (after core): Bulk import with column mapping

Do:
- [x] Analyze best way to stream large data and implement with .NET 8 features.
- [x] Add bulk import only after core is stable.
- [x] Provider-owned bulk implementation:
  - [x] SQL Server: `SqlBulkCopy` + `ColumnMappings` + streaming input
  - [x] PostgreSQL: `COPY` + explicit columns
  - [x] Oracle: array binding/bulk APIs + resource/cursor precautions

Exit criteria:
- High throughput, low memory; mapping validated; diagnostics returned; cancellation supported.

---

### Step 17 — CI/pipeline + vulnerability scanning (always on)

Do:
- [x] Ensure CI runs build, tests, analyzers (warnings-as-errors), SonarQube, `dotnet pack`, and NuGet/package validation.
- [x] Include NuGet vulnerability scanning as part of the quality gates.
- [x] Protect main/dev branches with the above checks.

Exit criteria:
- CI fails on analyzer/Sonar violations, vulnerability findings, or packaging issues.

## Embedded Source: IMPLEMENTATION_GUIDE.md (verbatim)

This section is included so implementation can be done without missing any requirements. If `IMPLEMENTATION_GUIDE.md` changes, regenerate this section by removing everything below this header and re-appending the latest `IMPLEMENTATION_GUIDE.md`.
